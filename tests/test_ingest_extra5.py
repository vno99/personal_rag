...
def test_ingest_file_with_missing_chunk_id(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ingest_weaviate, "get_embeddings", lambda: MagicMock(embed_documents=lambda texts: [[0.1]*10 for _ in texts]))
    chunk_file = tmp_path / "missing.jsonl"
    chunk_file.write_text(json.dumps({"content": "hello"}) + "\n", encoding="utf-8")
    ingest_weaviate.ingest_file(fake_coll, MagicMock(embed_documents=lambda texts: [[0.1]*10 for _ in texts]), chunk_file)
